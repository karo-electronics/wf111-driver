...
#define UF_ALLOC_NETDEV(_dev, _size, _name, _setup, _num_of_queues)     \
    do {                                                                \
        ...
        _dev = alloc_netdev_mq(_size, name, NET_NAME_UNKNOWN,           \
                                            _setup, _num_of_queues);    \
    } while (0);
#else /* linux version < 3.17.0 */
#define UF_ALLOC_NETDEV(_dev, _size, _name, _setup, _num_of_queues)     \
    do {                                                                \
        static char name[8];                                           \
        sprintf(name, "%s%s", UNIFI_NET_NAME, _name);                   \
...
uf_alloc_netdevice(CsrSdioFunction *sdio_dev, int bus_id)
{
    ...
    interfacePriv->patchMode = patch_mode;
#endif
...
void napi_init(struct net_device *dev, struct napi_struct *napi, struct sk_buff_head *napi_skb_list)
{
    skb_queue_head_init(napi_skb_list);

    netif_napi_add(dev, napi, napi_poll, CSR_WIFI_NAPI_BUDGET);
    napi_enable(napi);
}

void napi_deinit(struct napi_struct *napi, struct sk_buff_head *napi_skb_list)
{
    napi_disable(napi);
    netif_napi_del(napi);
    skb_queue_purge(napi_skb_list);
}

void napi_schedule_new_packets(unifi_priv_t *priv)
{
    int i;

    for (i = 0; i < CSR_WIFI_NUM_INTERFACES; i++)
    {
        struct netInterface_priv *ip = priv->interfacePriv[i];
        struct napi_struct *napi = &ip->napi;
        struct sk_buff_head *napi_skb_list = &ip->napi_skb_list;

        if (skb_queue_len(napi_skb_list))
        {
            napi_schedule(napi);
            local_bh_disable();
            local_bh_enable();
...
static void napi_enqueue(struct sk_buff_head *napi_skb_list, struct sk_buff *skb)
{
    skb_queue_tail(napi_skb_list, skb);
}

static struct sk_buff *napi_dequeue(struct sk_buff_head *napi_skb_list)
{
    return skb_dequeue(napi_skb_list);
}
...
static int napi_poll(struct napi_struct *napi, int budget)
{
    int work_done;
    struct sk_buff *skb;
    struct netInterface_priv *ip = container_of(napi, struct netInterface_priv, napi);

    for (work_done = 0; work_done < budget; work_done++)
    {
        skb = napi_dequeue(&ip->napi_skb_list);
...
unifi_rx(unifi_priv_t *priv, CSR_SIGNAL *signal, bulk_data_param_t *bulkdata)
{
    ...
    skb = (struct sk_buff*)bulkdata->d[0].os_net_buf_ptr;
    /*
      Reset the skb->len and let the skb_put to set both the skb->len value and skb->tail pointer appropriately
    */
    skb->len = 0;
            ...
            (interfacePriv->interfaceMode != CSR_WIFI_ROUTER_CTRL_MODE_P2PGO))
    {
       ...
       /* For AP GO mode, don't store the PDUs */
        if (port_action != CSR_WIFI_ROUTER_CTRL_PORT_ACTION_8021X_PORT_OPEN) {
            ...
            interfacePriv->stats.rx_dropped++;
            unifi_net_data_free(priv, &bulkdata->d[0]);
            unifi_trace(priv, UDBG1, "%s: Dropping packet, proto=0x%04x, %s port\n", __FUNCTION__,
...
static void process_ma_packet_ind(unifi_priv_t *priv, CSR_SIGNAL *signal, bulk_data_param_t *bulkdata)
{
    ...
    {
        unifi_trace(priv, UDBG1, "%s: MA-PACKET indication with status = %d\n",__FUNCTION__, pkt_ind->ReceptionStatus);
        unifi_net_data_free(priv,&bulkdata->d[0]);
        func_exit();
        return;
    }
